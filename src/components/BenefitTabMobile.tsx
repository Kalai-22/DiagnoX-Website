...
import { Zap, Shield, Lock, Smile, Bell } from 'lucide-react';
...
interface BenefitTabMobileProps {
  benefits: Array<{
    title: string;
    description: string;
    icon: 'zap' | 'shield' | 'lock' | 'smile' | 'bell';
  }>;
}
...
const iconMap = {
  ...
  smile: Smile,    // User-Friendly & Accessible
};
...
export function BenefitTabMobile({ benefits }: BenefitTabMobileProps) {
  ...
  // Icon button size: Mobile 52px, Tablet 52px-72px
  const iconButtonSize = isTabletMobileLayout
    ? Math.min(72, 52 + ((windowWidth - 601) / (1100 - 601)) * (72 - 52))
    : 52;

  // Icon size: Mobile 24px, Tablet 24px-32px
  const iconSize = isTabletMobileLayout
    ? Math.min(32, 24 + ((windowWidth - 601) / (1100 - 601)) * (32 - 24))
    : 24;

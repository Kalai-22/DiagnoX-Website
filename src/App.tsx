...
import imgCarWorkshopBg from 'figma:asset/fb49ace1073bc7e1df7f1b5b69f317e26e62b4cd.png';
...
function HeroSection() {
  ...
  const textTopPosition = isMobile ? '295px' : isSmallTab ? '255px' : '72px';
...
function AboutUsSection() {
  ...
  const totalSectionHeight = isMobile ? (860 + 752) : isSmallTab ? (860 + 752) : 1504;
      ...
      >
        ...
        <div className="mx-auto max-w-7xl" style={{ 
          ...
          >
            ...
            {/* Workflow steps */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: `${24 * scaleFactor}px` }} key={animationKey}>
              {steps.map((step, index) => {
                // Sequential animation for mobile/tablet
                if (useSequentialAnimation) {
                  // Animation timing configuration
                  // Total cycle: 10.2s (5 steps × 1.2s glow + 0.3s gaps + 3s pause)
                  const totalDuration = 10.2;
                  const glowDuration = 1.2;
                  const startTime = index * 1.5;
                  const endTime = startTime + glowDuration;
                      ...
                      <motion.div
                        ...
                        animate={{
                          backgroundColor: theme === 'dark'
                            ? ['rgba(255, 255, 255, 0.6)', 'rgba(255, 255, 255, 0.6)', 'rgba(255, 255, 255, 1)', 'rgba(255, 255, 255, 0.6)', 'rgba(255, 255, 255, 0.6)']
                            : ['rgba(0, 0, 0, 0.6)', 'rgba(0, 0, 0, 0.6)', 'rgba(0, 0, 0, 1)', 'rgba(0, 0, 0, 0.6)', 'rgba(0, 0, 0, 0.6)'],
                        }}
                        transition={{
                          duration: totalDuration,
                          repeat: Infinity,
                          ease: 'easeInOut',
                          times: [0, startTime / totalDuration, (startTime + glowDuration / 2) / totalDuration, endTime / totalDuration, 1],
                          repeatType: 'loop',
                        }}
                      />
                ...
                return (
                  ...
                  >
                    ...
                    {/* Text */}
                    <p className={`${FONTS.body} transition-all duration-300 group-hover:scale-105 origin-left ${
      ...
      >
        ...
        <div className="relative z-10 mx-auto h-full flex flex-col justify-between">
          ...
          <div className="max-w-[1440px] mx-auto w-full px-6 pb-[40px]">
            {isMobile || (windowWidth >= 601 && windowWidth <= 1100) ? (
...
function WaitlistSection({ onSupportClick, onLearnMoreClick }: { onSupportClick: () => void; onLearnMoreClick: () => void }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetch('https://script.google.com/macros/s/AKfyc...your-script-id.../exec', {
      method: 'POST',
